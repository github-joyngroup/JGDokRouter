...
namespace Joyn.DokRouter
{
    public class EngineMonitor : BackgroundService
    {
        ...
        public static void MainMonitor(CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var firstPageResult = _dokRouterDAL.GetRunningInstances(1);

                    var allPagesTasks = Enumerable.Range(2, firstPageResult.lastPage).Select(pageNumber =>
                    {
                        return Task.Run(() =>
                        {
                            return _dokRouterDAL.GetRunningInstances(pageNumber);
                        });
                    }).ToArray();
                    ...
                    //Fill expired fields
                    foreach (var pipelineInstance in baseRunningInstances)
                    {
        ...
        private static void FillExpired(PipelineInstance pipelineInstance, Action<PipelineInstance, ActivityInstance, ActivityExecution> onExpiredActivityTry)
        {
            ...
            if (!pipelineInstance.ActivityInstances.Any())
            {
                //Return - Pipeline has no activities yet
                return;
            }

            //As we sequentially execute the activities, we can just check the last activity instance, as it will be the one that is currently being executed
            var lastActivityInstance = pipelineInstance.ActivityInstances[pipelineInstance.ActivityInstances.Keys.Max()];
            if (lastActivityInstance.ActivitySLAMoment < DateTime.UtcNow)
            {
                lastActivityInstance.ActivitySLAExpiredBy = DateTime.UtcNow - lastActivityInstance.ActivitySLAMoment;
            }

            foreach (var activityExecution in lastActivityInstance.Executions.Where(e => !e.EndedAt.HasValue))
            {
                if (activityExecution.ActivityTrySLAMoment < DateTime.UtcNow)
                {
                    ...
                    onExpiredActivityTry(pipelineInstance, lastActivityInstance, activityExecution);
                }
            }
        }
